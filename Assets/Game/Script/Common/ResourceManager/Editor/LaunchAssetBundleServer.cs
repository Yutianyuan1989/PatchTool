...
namespace AssetBundles
{
    ...
    {
        const string kLocalAssetbundleServerMenu = "Tools/AssetBundle/Local AssetBundle Server";
        ...
        static void Run()
        {
            string pathToAssetServer = Path.GetFullPath("Assets/WGame/Scripts/Common/ResourceManager/Editor/AssetBundleServer.exe");
        ...
        static string GetMonoProfileVersion()
        {
            ...
            for (int i = 0; i < foldersWithApi.Length; i++)
            {
                ...
                if(string.CompareOrdinal(foldersWithApi[i], profileVersion)>0)
                {
